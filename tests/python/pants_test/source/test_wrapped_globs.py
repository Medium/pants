...
class FilesetRelPathWrapperTest(BaseTest):
  ...
  def test_no_dir_glob(self):
    self.add_to_build_file('y/PANTS.BUILD', 'dummy_target(name="y", sources=globs("*"))')
    self.context().scan()
  ...
  def _spec_test(self, spec, expected):
    self.add_to_build_file('y/PANTS.BUILD', 'dummy_target(name="y", sources={})'.format(spec))
  ...
  def test_glob_exclude(self):
    self.add_to_build_file('y/PANTS.BUILD', dedent("""
  ...
  def test_glob_with_folder_with_only_folders(self):
    self.add_to_build_file('z/PANTS.BUILD', dedent("""
      dummy_target(name="z", sources=globs("*", exclude=["PANTS.BUILD"]))
      """))
    graph = self.context().scan()
    self.assertEqual([],
  ...
  def test_subdir_glob(self):
    self.add_to_build_file('y/PANTS.BUILD', 'dummy_target(name="y", sources=globs("dir/*.scala"))')
    self.context().scan()
  ...
  def test_parent_dir_glob(self):
    self.add_to_build_file('y/PANTS.BUILD', 'dummy_target(name="y", sources=globs("../*.scala"))')
    with self.assertRaises(AddressLookupError):
      self.context().scan()
  ...
  def test_absolute_dir_glob(self):
    self.add_to_build_file('y/PANTS.BUILD', 'dummy_target(name="y", sources=globs("/root/*.scala"))')
    with self.assertRaises(AddressLookupError):
      self.context().scan()
  ...
  def test_rglob_follows_symlinked_dirs_by_default(self):
    self.add_to_build_file('z/w/PANTS.BUILD', 'dummy_target(name="w", sources=rglobs("*.java"))')
    graph = self.context().scan()
  ...
  def test_rglob_respects_follow_links_override(self):
    self.add_to_build_file('z/w/PANTS.BUILD',
