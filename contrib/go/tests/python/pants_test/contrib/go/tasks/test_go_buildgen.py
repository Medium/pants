...
class GoBuildgenTest(TaskTestBase):
  ...
  def test_stitch_deps_generate_builds(self):
    ...
    self.assertEqual({'src/go/jane/PANTS.BUILD'}, self.buildroot_files() - pre_execute_files)

  def test_stitch_deps_generate_builds_custom_extension(self):
    self.set_options(extension='.gen')
    pre_execute_files = self.stitch_deps_local(materialize=True)
    # NB: The src/go/fred/PANTS.BUILD file on disk was deleted and replaced with src/go/fred/PANTS.BUILD.gen.
    self.assertEqual({'src/go/fred/PANTS.BUILD.gen', 'src/go/jane/PANTS.BUILD.gen'},
  ...
  def test_stitch_deps_remote_unused(self):
    ...
    self.assertIn('3rdparty/go/github.com/user/repo/PANTS.BUILD', self.buildroot_files())
    self.assertEqual(pre_execute_files, self.buildroot_files())
  ...
  def test_stitch_deps_remote_existing_rev_respected(self):
    ...
    self.assertEqual({'src/go/jane/PANTS.BUILD', '3rdparty/go/pantsbuild.org/fake/PANTS.BUILD'},
                     self.buildroot_files() - pre_execute_files)
